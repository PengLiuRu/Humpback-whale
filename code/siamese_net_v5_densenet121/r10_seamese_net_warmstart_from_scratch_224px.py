...
def get_tagged_data():
    ...
    playground = dict([(p[:-4] + '.jpg', w) for _, p, w in read_csv(INPUT_PATH + 'playground\\train.csv').to_records()])
...
p2bb = get_boxes()                   #train  test 和 playground的所有图片边框
tagged = get_tagged_data()           #playground/train.csv  和 train.csv的所有图片和ID
w2p = get_whale_lists_for_class_dict(tagged)    #为所有tagged图片分类
print_stats(w2p)

# test
submit = [p for _, p, _ in read_csv(INPUT_PATH + 'sample_submission.csv').to_records()]
submit = [f[:-4] + '.jpg' for f in submit]
join = set(list(tagged.keys()) + submit)               #playground/train.csv  和 train.csv和sample_submission.csv的所有图片名集合
...
def get_kfold_split_playground(w2p):
    ...
    fold_path = OUTPUT_PATH + 'kfold\\new_4_folds_split_train_val_v1.pkl'
    data = load_from_file_fast(fold_path)
    data = data['fold_{}'.format(0)]

    train_part = []
    train = read_csv(INPUT_PATH + 'playground\\train.csv')
...
train, new_train, val = get_kfold_split_playground(w2p) #train_part:playground/train.csv中图片数>=2且不是new_whale的图片名（.jpg）集合。
                                                        #new_train_part, valid_part：'kfold/new_4_folds_split_train_val_v2.pkl'中fold_0的train和valid
w2ts = gen_train_labels_to_fotos(w2p, train)            #如果w2p[k]中图片存在在train中，就把它放入w2ts[k]中
...
class TrainingDataFast(Sequence):
    def __init__(self, score_list, steps=1000, batch_size=32):  #res , 20 ,12
        """
    ...
    def on_epoch_end(self):
        ...
        if segment:
            ...
            tmp   = []
            batch = math.ceil(self.score.shape[0] / LINEAR_ASSIGNMENT_SEGMENT_SIZE)          #6280/1 = 6280
            ...
            for start in range(0, self.score.shape[0], batch):
                ...
                tmp.append(x + start)
            x = np.concatenate(tmp)             #tmp 为线性分配后每行的地址索引 array([   1,    2,    0, ..., 6819, 6605, 6815])
        else:
        ...
        for ts in w2ts.values():
            ...
            for ab in zip(ts,d): self.match.append(ab)       #match = [('000466c4.jpg', '2c4813a1.jpg'),  train中为同一类的图片形成一对
        ...
        # Force a different choice for an eventual next epoch.为这一次选中的鲸鱼设置一个较大的值——消除了这种潜在的配对，防止下次选中他们
...
class FeatureGen(Sequence):
    ...
    def __getitem__(self, index):
        ...
        res = preprocess_image(res)     #图片预处理
...
class ScoreGen(Sequence):
    def __init__(self, x, y=None, batch_size=2048, verbose=1):
        ...
        if y is None:
            self.y           = self.x
            self.ix, self.iy = np.triu_indices(x.shape[0],1)            #ix,iy = [0, 0, 1],[1, 2, 2]
        else:
    ...
    def __getitem__(self, index):
        ...
        a     = self.y[self.iy[start:end],:]                #features[0, 0, 1]
        b     = self.x[self.ix[start:end],:]                 #features[1. 2. 2]
        #if self.verbose > 0: 
...
def compute_score_fast(verbose=1):
    ...
    """
    res = []
    batch = int(math.ceil(len(train) / LINEAR_ASSIGNMENT_SEGMENT_SIZE))     #len(train) = 6280   ,LINEAR_ASSIGNMENT_SEGMENT_SIZE = 1
    for start in range(0, len(train), batch):
        end = min(len(train), start + batch)
        train_batch = train[start:end]

        features = branch_model.predict_generator(FeatureGen(train_batch, verbose=verbose), max_queue_size=12, verbose=0, use_multiprocessing=False)  #features = （n,1024)
...
def make_steps_fast(step, ampl, verbose=2):
    ...
    for i in range(len(score_list)):
        score_list[i] += ampl*np.random.random_sample(size=score_list[i].shape)    #numpy.random.random_sample(size=None)：生成一个[0,1)之间随机浮点数或N维浮点数组
    
    # Train the model for 'step' epochs
    history = model.fit_generator(
        TrainingDataFast(score_list, steps=step, batch_size=12),
...
if __name__ == '__main__':
    ...
    make_steps_fast(20, 1)
    score_valid = run_validation(thr)
    model.save(MODELS_PATH + '{}/{}_{}_temp0_{:.6f}.model'.format(saved_folder, model_str, version, score_valid))
    ...
    score_valid = run_validation(thr)
    model.save(MODELS_PATH + '{}/{}_{}_temp1_{:.6f}.model'.format(saved_folder, model_str, version, score_valid))
    ...
    set_lr(model, 1e-5)
    for _ in range(4): make_steps_fast(5, 0.25)
    score_valid = run_validation(thr)
